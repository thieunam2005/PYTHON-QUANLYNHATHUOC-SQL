...
def show_invoice_tab(parent):
    ...
    def delete_invoice(ma_hd):
        confirm = messagebox.askyesno("Xác nhận", f"Bạn có chắc muốn xoá hoá đơn {ma_hd}?")
        if confirm:
            conn = get_connection()
            cursor = conn.cursor()
            cursor.execute("DELETE FROM CT_HOADON WHERE maHoaDon = ?", ma_hd)
            cursor.execute("DELETE FROM HOADON WHERE maHoaDon = ?", ma_hd)
            conn.commit()
            conn.close()
            messagebox.showinfo("Thành công", "Đã xoá hoá đơn")
            render_table()
    ...
    def render_table(customer_filter="", from_date="", to_date=""):
        ...
        for invoice in invoices:
            ...
            tk.Button(row_frame, text="Xoá", bg="red", fg="white", command=lambda m=ma: delete_invoice(m)).pack(anchor="e", padx=10, pady=(0, 5))
